...
namespace Sample05
{
    class Program
    {
        ...
        static void test_insert()
        {
            ...
            using (var conn = new SqlConnection("Data Source=127.0.0.1;User ID=sa;Password=ok;Initial Catalog=Sample;Pooling=true;Max Pool Size=100;"))
            {
        ...
        static void test_select_content_with_comment()
        {
            using (var conn = new SqlConnection("Data Source=127.0.0.1;User ID=sa;Password=ok;Initial Catalog=Sample;Pooling=true;Max Pool Size=100;"))
            {
                ...
                using (var result = conn.QueryMultiple(sql_insert, new { id = 5 }))
                {
                    ...
                    Console.WriteLine($"test_select_content_with_comment:内容5的评论数量{content.comments.AsList<Comment>().Count}");
                }
